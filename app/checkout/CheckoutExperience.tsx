...
function readAssistantContext() {
  try {
    return sessionStorage.getItem(PROJECT_CONTEXT_KEY) ?? "";
  } catch {
    return "";
  }
}

function subscribeToAssistantContext(onChange: () => void) {
  function onStorage(event: StorageEvent) {
    if (event.key === PROJECT_CONTEXT_KEY || event.key === null) onChange();
  }
...
export default function CheckoutExperience({
  ...
  const storedContext = useSyncExternalStore(
    subscribeToAssistantContext,
    readAssistantContext,
    emptyAssistantContext,
  );
  const [editedDescription, setProjectDescription] = useState<string | null>(
    null,
  );
  const [contextDismissed, setContextDismissed] = useState(false);
  const projectDescription = editedDescription ?? storedContext;
  const hasAssistantContext = Boolean(storedContext) && !contextDismissed;
  ...
  function selectProduct(nextId: ProductId | "") {
    ...
    window.history.replaceState(
      null,
      "",
      nextId ? `/checkout?product=${nextId}` : "/checkout",
    );
  }
    ...
    return (
      <main className="checkout-page flex min-h-screen items-center px-5 py-12 text-white sm:px-6">
        <div className="mx-auto w-full max-w-xl rounded-2xl border border-white/10 bg-white/[0.035] p-7 text-center sm:p-9">
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full border border-emerald-300/20 bg-emerald-300/10 text-lg text-emerald-300">
            ✓
          </div>
          ...
          <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-white/50">
            We&apos;ll review your project and reply with the clearest next
            step.
          </p>
    ...
    <main className="checkout-page min-h-screen px-5 py-7 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl">
        <header className="flex items-center justify-between gap-4">
          ...
          >
            <span className="flex h-9 w-9 items-center justify-center rounded-lg border border-white/15 bg-white/[0.06] text-xs font-semibold">
              YY
            </span>
            <span>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/[0.025] p-5 sm:p-6">
            ...
            <label className="mt-4 block text-sm font-medium text-white/60">
              ...
              <select
                value={productId}
                onChange={(event) =>
                  selectProduct(event.target.value as ProductId | "")
                }
            ...
            {product ? (
              ...
              <p className="mt-5 border-t border-white/10 pt-5 text-sm leading-6 text-white/45">
            ...
            <div className="mt-5 border-t border-white/10 pt-5 text-sm leading-6 text-white/45">
              ...
              <p className="mt-1">
                No payment is taken on this page. We confirm the project first
                and send payment details afterward.
              </p>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 sm:p-6">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-white/45">
                Short details are enough. We&apos;ll reply with the next step.
              </p>
            </div>
            ...
            >
              ...
              <p className="text-center text-[11px] leading-5 text-white/25">
                No payment is charged when you send this form.
              </p>
            </form>
          </section>
        </div>
