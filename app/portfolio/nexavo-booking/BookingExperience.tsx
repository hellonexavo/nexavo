...
export default function BookingExperience({ currentYear }: { currentYear: number }) {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#f4f7f7] text-slate-950 selection:bg-cyan-200">
      ...
      <div id="main">{view === "customer" ? <BookingFlow language={language} onConfirm={addBooking} onDashboard={() => showView("dashboard")} /> : <OwnerDashboard language={language} bookings={bookings} filter={filter} onFilter={setFilter} onStatus={updateStatus} onReset={resetDashboard} />}</div>
