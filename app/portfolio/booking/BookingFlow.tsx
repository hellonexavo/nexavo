    ...
    <form ref={formRef} onSubmit={(event) => event.preventDefault()} noValidate>
      <section className="relative overflow-hidden px-5 pb-20 pt-16 sm:px-7 sm:pb-28 sm:pt-24 lg:px-10">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1fr_0.72fr] lg:items-center">
          ...
          <div className="rounded-[32px] border border-white bg-white/80 p-6 shadow-[0_30px_90px_rgba(15,23,42,0.10)] backdrop-blur sm:p-8"><div className="flex items-center justify-between"><span className="rounded-full bg-cyan-50 px-3 py-1.5 text-xs font-bold text-cyan-800">Live demo</span><span className="text-sm text-slate-400">01 / 04</span></div><p className="mt-10 text-sm text-slate-500">{t.estimate}</p><p className="invisible mt-2 text-5xl font-semibold tracking-tight text-slate-950" aria-hidden="true">{currency(price)}</p><div className="mt-8 grid grid-cols-2 gap-3 text-sm"><div className="rounded-2xl bg-slate-50 p-4"><p className="text-slate-400">{t.service}</p><p className="mt-1 font-semibold text-slate-800">{t[service]}</p></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-slate-400">{t.size}</p><p className="mt-1 font-semibold text-slate-800">{size} m²</p></div></div></div>
        </div>
      </section>
