...
export default function BookingFlow({ onConfirm, onDashboard }: Props) {
  ...
  return <main>
    ...
    <section id="services" className="studio-section studio-shell scroll-mt-24">
      <div className="studio-section-heading"><div><p className="studio-eyebrow">The studio menu</p><h2>Carefully considered services.</h2></div><p>Sample services, pricing, and availability demonstrate how the template can present a real business offer.</p></div>
      <div className="studio-service-grid">{services.map((item) => <article key={item.id} className="studio-service-card"><div className="studio-service-image"><Image src="/yy-booking/studio-hero.png" alt="" fill sizes="(max-width: 700px) 100vw, (max-width: 1100px) 50vw, 33vw" style={{ objectPosition: item.imagePosition }} /></div><div className="studio-service-body"><div className="studio-service-meta"><span>{item.duration} min</span><span>{money(item.price)}</span></div><h3>{item.name}</h3><p>{item.description}</p><button type="button" onClick={() => openBooking(item.id)} className="studio-text-button">Book this service <span>↗</span></button></div></article>)}</div>
    </section>
    ...
    <section id="booking" ref={bookingRef} className="studio-booking-section scroll-mt-20">
      <div className="studio-shell">
        ...
        <div className="studio-wizard" key={step}>
          {step === 1 && <StepPanel eyebrow="Step 1" title="Choose your service" copy="Select the experience that best fits your visit."><div className="studio-choice-grid">{services.map((item) => <Choice key={item.id} active={service === item.id} onClick={() => setService(item.id)} title={item.name} meta={`${item.duration} min · ${money(item.price)}`} copy={item.description} />)}</div></StepPanel>}
          ...
          {step === 5 && <StepPanel eyebrow="Step 5" title="Review your booking" copy="Confirm the demo details below. No payment is requested."><Review booking={{ service: selectedService.name, specialist: selectedSpecialist.name, date, time, customer: customer.name, duration: `${selectedService.duration} minutes`, price: money(selectedService.price) }} /><div className="studio-notice">This is a simulated appointment. A production template requires a connected booking backend, database, email, or calendar service.</div></StepPanel>}
          {step === 6 && confirmed && <div className="studio-confirmation"><span className="studio-confirmation-icon">✓</span><p className="studio-eyebrow">Booking confirmed · Demo</p><h2>We’ll see you at the studio.</h2><p>A simulated confirmation has been prepared for <strong>{confirmed.customer}</strong>. No real appointment was created and no message was sent.</p><Review booking={{ service: selectedService.name, specialist: selectedSpecialist.name, date, time, customer: confirmed.customer, duration: `${selectedService.duration} minutes`, price: money(selectedService.price) }} /><div className="studio-reference"><span>Demo reference</span><strong>{confirmed.reference}</strong></div><div className="studio-actions justify-center"><button type="button" onClick={onDashboard} className="studio-button studio-button-primary">View owner preview <span>→</span></button><button type="button" onClick={reset} className="studio-button studio-button-secondary">Book another demo</button></div></div>}
