...
function parseMessages(value: unknown): AssistantMessage[] | null {
  ...
  const messages = value.filter(
    (item): item is AssistantMessage =>
      ...
      (item as AssistantMessage).content.length <= 1200,
  );
...
function getProviderConfig() {
  ...
  return {
    apiKey,
    apiUrl: process.env.AI_API_URL || "https://ai-gateway.vercel.sh/v1/chat/completions",
...
export async function GET() {
  ...
  return Response.json({ ok: true, aiConfigured: Boolean(apiKey), model: apiKey ? model : null });
}
...
export async function POST(request: Request) {
  ...
  if (!apiKey) {
    return Response.json({
      reply: fallbackAssistantReply(messages.at(-1)!.content),
      mode: "demo",
    });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `AI provider returned ${response.status}${detail ? `: ${detail.slice(0, 240)}` : ""}`,
      );
    }
    ...
    console.error(
      "YY Assistant provider request failed",
